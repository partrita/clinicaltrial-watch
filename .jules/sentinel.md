...
## 2025-08-05 - Type Confusion in Nested API Responses (CWE-400)
**Vulnerability:** The `process_trial` function extracted metadata from deep within the ClinicalTrials.gov API response using chained `.get()` calls without validating the types of intermediate structures. A malformed or malicious API response containing a string or list where a dictionary was expected would cause `AttributeError` or `TypeError` crashes, leading to a Denial of Service.
**Learning:** Validating only the top-level type of an API response is insufficient when performing deep extraction. Type confusion in nested structures can bypass standard error handling and crash the application during property access or indexing.
**Prevention:** Always perform explicit type validation (`isinstance(..., dict)`) on every intermediate nested module or structure before attempting to access its children or attributes, especially when dealing with external data sources.
...
**Prevention:** Capture the file mode (`st_mode`) of the existing file before replacement and apply it to the temporary file using `os.chmod`. Always include error handling (e.g., `try...except OSError`) when applying permissions to account for different filesystems or restricted environments.
