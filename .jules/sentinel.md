...
## 2026-06-06 - Insecure Default TLS Version in HTTP Clients (CWE-327)
**Vulnerability:** The application used `requests.Session` with default settings for outgoing HTTPS requests to ClinicalTrials.gov. Depending on the environment's OpenSSL configuration, this could allow the negotiation of insecure or deprecated TLS versions (e.g., TLS 1.0 or 1.1), exposing the application to man-in-the-middle attacks.
**Learning:** Default library configurations often prioritize compatibility over security. For critical outbound traffic to known modern APIs, security should be enforced at the application layer by explicitly specifying minimum protocol versions.
**Prevention:** Enforce TLS 1.2 or higher for all outbound HTTPS requests by mounting a custom `HTTPAdapter` that configures a secure `SSLContext`. Centralize this implementation to ensure consistency across all modules performing network I/O.
