...
## 2026-08-12 - Denial of Service via Deeply Nested JSON (CWE-400)
**Vulnerability:** The application was vulnerable to Denial of Service (DoS) when parsing maliciously crafted or malformed JSON data from the ClinicalTrials.gov API. While the response size was capped at 10MB, the standard `json` module can still trigger a `RecursionError` and crash the process if the JSON nesting depth exceeds Python's recursion limit.
**Learning:** Resource limits on input size (e.g., Content-Length) are necessary but not sufficient to prevent DoS. Structural complexity, such as nesting depth in JSON or YAML, must also be handled gracefully to prevent stack exhaustion or excessive CPU usage during parsing.
**Prevention:** Always wrap JSON and YAML parsing of untrusted data (from APIs or local files) in `try...except RecursionError` blocks. Ensure the application fails securely by logging the error and returning a safe default value or raising a controlled exception instead of crashing.
