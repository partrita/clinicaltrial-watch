...
<a href="../data/targets/"""
        + target_lower
        + r"""/all_trials_raw.csv" class="btn btn-primary" role="button" aria-label="Download full trial data in CSV format">📥 Download Full Data (CSV)</a>
<a href="../data/targets/"""
        + target_lower
        + r'''/status_summary.csv" class="btn btn-outline-secondary" role="button" aria-label="Download status summary in CSV format">📥 Download Status Summary (CSV)</a>
...
if os.path.exists(summary_path):
    ...
    for item in summary:
        ...
        full_sponsor_escaped = escape_html(sponsor)
        if len(sponsor) > 30:
            display_sponsor = escape_html(sponsor[:30] + "...")
            safe_sponsor = f'<span title="{full_sponsor_escaped}">{display_sponsor}</span>'
        else:
            safe_sponsor = full_sponsor_escaped

        conditions = item.get('conditions', 'N/A')
        full_conditions_escaped = escape_html(conditions)
        if len(conditions) > 30:
            display_conditions = escape_html(conditions[:30] + "...")
            safe_conditions = f'<span title="{full_conditions_escaped}">{display_conditions}</span>'
        else:
            safe_conditions = full_conditions_escaped

        trial_id = item['id']
        # NCT Link with aria-label for accessibility
        nct_link = f'[{trial_id}](https://clinicaltrials.gov/study/{trial_id}){{aria-label="View trial {trial_id} on ClinicalTrials.gov"}}'

        print(f"| {nct_link} | {safe_sponsor} | {update_badge} | {status_badge} | {safe_conditions} | {escape_html(item.get('phases', 'N/A'))} | {escape_html(item.get('study_start', 'N/A'))} | {escape_html(item.get('study_end', 'N/A'))} | {escape_html(item.get('enrollment', 'N/A'))} | {escape_html(item.get('last_updated', 'N/A'))} |")
    print('</div>')
else:
